...
import {Monitor,WalletCards,LockKeyhole,UnlockKeyhole,ArrowDownCircle,ArrowUpCircle,ReceiptText,Printer,Clock3,Scale,Landmark,Coins,TriangleAlert} from 'lucide-react'
...
export default async function Page(){
 ...
 const [current,history,dayPayments]=await Promise.all([
 ...
 return <AppShell>
  <div className="v6-title-row"><div><div className="eyebrow">POS & CASH CONTROL · V14 ULTIMATE ERP</div><h1>Caisse professionnelle</h1><p>Encaissements, fond de caisse, sorties, contrôle des écarts et rapport X/Z.</p></div><div className="detail-actions"><Link className="btn secondary" href="/cash/report"><Printer size={17}/> Rapport X / Z</Link><Link className="btn secondary" href="/settings/print"><ReceiptText size={17}/> Impression</Link></div></div>
  ...
  {!current?<section className="card gap-top"><div className="section-title"><div><h2><UnlockKeyhole size={19}/> Ouvrir la caisse</h2><p>Démarrez une session avant les encaissements espèces. Le fond initial sert de base au contrôle de fin de journée.</p></div></div><form action="/api/cash/open" method="post" className="v11-grid two"><label>Fond de caisse €<input name="openingAmount" type="number" step="0.01" min="0" defaultValue="100"/></label><label>Note d’ouverture<input name="note" placeholder="Ex. ouverture matin / équipe"/></label><div className="span2"><button className="btn"><UnlockKeyhole size={16}/> Ouvrir la caisse</button></div></form></section>:<>
   <section className="card gap-top"><div className="section-title"><div><h2><Clock3 size={19}/> Session en cours</h2><p>Ouverte par {current.openedBy.name} · {dateFr(current.openedAt)}</p></div><span className="badge large status-ready">OUVERTE</span></div>
