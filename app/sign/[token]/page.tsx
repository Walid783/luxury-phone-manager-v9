...
export default async function PublicSign({params}:{params:Promise<{token:string}>}){
 ...
 if(!r)notFound()
 const isImage=!!r.customerSignature?.startsWith('data:image/')
 return <main style={{minHeight:'100vh',background:'#f4f7fb',padding:'32px 18px',fontFamily:'Arial,sans-serif',color:'#142033'}}>
  <div style={{maxWidth:760,margin:'0 auto'}}>
   ...
   <section style={{background:'#fff',border:'1px solid #dfe6ef',borderRadius:18,padding:24,boxShadow:'0 10px 30px rgba(15,23,42,.06)'}}>
    ...
    {r.customerSignature?<div style={{padding:18,borderRadius:14,background:'#ecfdf3',border:'1px solid #b7ebc6'}}><b>✅ Document déjà signé</b>{isImage?<img src={r.customerSignature} alt="Signature client" style={{display:'block',maxWidth:'100%',height:120,marginTop:12,objectFit:'contain',objectPosition:'left center',background:'#fff',borderRadius:10}}/>:<div style={{marginTop:6}}>{r.customerSignature}</div>}<small>{r.signatureAt?dateFr(r.signatureAt):''}</small></div>:<form action={`/api/public/sign/${token}`} method="post" style={{display:'grid',gap:14}}>
     <label style={{fontWeight:700}}>Nom et prénom du client<input name="customerName" required autoFocus style={{display:'block',width:'100%',marginTop:8,padding:'15px 16px',borderRadius:12,border:'1px solid #cfd8e3',fontSize:18}} placeholder="Votre nom complet"/></label>
     <SignaturePad />
